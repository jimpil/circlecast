...
(defn filter-index
  ...
  (for [ pred-clause predicate-clauses
        :let [[lvl1-prd lvl2-prd lvl3-prd] (apply (impl/from-eav index) pred-clause)] ; predicates for the first and second level of the index, also keeping the path to later use its meta
...
(defn single-index-query-plan
  ...
  [query indx db]
  (let [q-res (query-index (impl/indx-at db indx) query)]
    (bind-variables-to-query q-res  (impl/indx-at db indx))))
