...
(defn multi-pivot
  ...
  (when (even? (count fns))
    (let [reduce-help (fn [a-fn accum-val new-val] (a-fn accum-val new-val))
          fn-pairs      (partition 2 fns)
          mapping-fns   (map first fn-pairs)
          reduction-fns (map second fn-pairs)
          mapped-tuples (map #(hash-map (grouping-fn %) ((apply juxt mapping-fns) %)) coll)]
      (apply merge-with
             (fn [accum-vec new-vec]
               (mapv reduce-help reduction-fns accum-vec new-vec))
             mapped-tuples))))

(defn pivot
  ([coll grouping-fn]
   (pivot coll grouping-fn always1 +))
  ([coll grouping-fn mapping-fn]
   (pivot coll grouping-fn mapping-fn +))
  ([coll grouping-fn mapping-fn reduce-fn]
   (ut/map-vals first (multi-pivot coll grouping-fn mapping-fn reduce-fn))))
...
(defn join*
  ...
  ([join-style left-coll right-coll join-fn]
   (join* join-style left-coll right-coll join-fn join-fn))
...
(defmacro defjoin
  ...
  `(defn ~join-name
     ...
     ([~'left-coll ~'right-coll ~'left-join-fn ~'right-join-fn]
      (join* ~join-style ~'left-coll ~'right-coll ~'left-join-fn ~'right-join-fn))))

(defjoin inner-join
         inner-style
         "This is for performing inner joins.
          The join value must exist in both lists.")
(defjoin left-outer-join
         left-outer-style
         "This is for performing left outer joins.
          The join value must exist in the left hand list.")
(defjoin right-outer-join
         right-outer-style
         "This is for performing right outer joins.
          The join value must exist in the right hand list.")
(defjoin full-outer-join
         full-outer-style
         "This is for performing full outer joins.
          The join value may exist in either list.")

(defn natural-join
  "Performs the natural join.
   If there are no keys that intersect, the empty vector is returned."
...
(defn cross-join
  ...
  [left-coll right-coll]
  (inner-join left-coll right-coll always1))
